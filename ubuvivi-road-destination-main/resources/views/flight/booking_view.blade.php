...
@section('title')
    {{ $booking->names }} - Flight Booking - Ubuvivi Tours
@endsection
                        ...
                        <div class="card-body px-0">
                            <div class="row no-gutters">
                                <div class="col-12 border-bottom py-3">
                                    <h6 class="bg-primary font-primary text-white p-2 mb-2">Flight Route</h6>
                                    <p class="mb-1 font-15 px-2 font-primary">
                                        <strong>{{ $booking->departure_airport ?? 'N/A' }}</strong> → <strong>{{ $booking->arrival_airport ?? 'N/A' }}</strong>
                                    </p>
                                </div>
                                <div class="col-12 border-bottom py-3">
                                    <h6 class="bg-primary font-primary text-white p-2 mb-2">Trip Type</h6>
                                    <p class="mb-1 font-15 px-2 font-primary">
                                        {{ ucfirst($booking->trip_type ?? 'N/A') }}
                                    </p>
                                </div>
                                <div class="col-12 border-bottom py-3">
                                    <h6 class="bg-primary font-primary text-white p-2 mb-2">Flight Class</h6>
                                    <p class="mb-1 font-15 px-2 font-primary">
                                        {{ $booking->flight_class_label ?? 'N/A' }}
                                    </p>
                                </div>
                                <div class="col-12 border-bottom py-3">
                                    <h6 class="bg-primary font-primary text-white p-2 mb-2">Departure Date</h6>
                                    <p class="mb-1 font-15 px-2 font-primary">
                                        {{ $booking->departure_date?->format('M d, Y') ?? 'N/A' }}
                                    </p>
                                </div>
                                @if($booking->trip_type === 'round' && $booking->return_date)
                                    <div class="col-12 border-bottom py-3">
                                        <h6 class="bg-primary font-primary text-white p-2 mb-2">Return Date</h6>
                                        <p class="mb-1 font-15 px-2 font-primary">
                                            {{ $booking->return_date?->format('M d, Y') ?? 'N/A' }}
                                        </p>
                                    </div>
                                @endif
                                <div class="col-12 border-bottom py-3">
                                    <h6 class="bg-primary font-primary text-white p-2 mb-2">Number of Passengers</h6>
                                    <p class="mb-1 font-15 px-2 font-primary">
                                        {{ $booking->number_of_passengers ?? 'N/A' }}
                                    </p>
                                </div>
                                @if($booking->additional_info)
                                    <div class="col-12 py-3">
                                        <h6 class="bg-primary font-primary text-white p-2 mb-2">Additional Information</h6>
                                        <p class="mb-1 font-15 px-2 font-primary">
                ...
                <div class="col-12 col-md-6 col-xl-5">
                    ...
                    <div class="bg-dark-1 py-4 rounded">
                        <div class="row m-auto" style="max-width:600px">
                            ...
                            @if($booking->airline)
                                <div class="form-group col-sm-12">
                                    <h4 class="input_title">Airline:</h4>
                                    {!! Form::text('airline', $booking->airline, ['class' => 'form-control', 'placeholder' => 'Airline', 'readonly' => true]) !!}
                                </div>
                            @endif
